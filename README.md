...
Für Arch-basierte Distributionen liegt ein `PKGBUILD` bei:
...
Das Paket installiert den Launcher als `github-manager` (`/usr/bin/github-manager`)
und kann danach einfach per `github-manager` gestartet werden. Vor dem Bauen
müssen im `PKGBUILD` `pkgver`, `url` und der Maintainer-Eintrag an das
tatsächliche Repository angepasst werden, da GitHub erst zu einem existierenden
Git-Tag (`v$pkgver`) ein Quell-Archiv anbietet.
